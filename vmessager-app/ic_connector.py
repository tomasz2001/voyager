...
class VMessageConnector:
    ...
    def _create_agent(self, pem_path: Path) -> Agent:
        ...
        """
        if pem_path.exists():
            print(f"{Fore.YELLOW}INFO: Znaleziono istniejącą tożsamość w: {pem_path}")
            identity = Identity.from_pem(pem_path.read_text())
        else:
            print(f"{Fore.YELLOW}INFO: Nie znaleziono tożsamości, tworzę nową w: {pem_path}")
            identity = Identity()
            pem_path.parent.mkdir(parents=True, exist_ok=True)
            pem_path.write_text(identity.to_pem().decode("utf-8"))
        
        print(f"{Fore.GREEN}INFO: Principal ID agenta: {identity.sender().to_str()}")
    ...
    async def _execute_call(self, method_type: str, method_name: str, params: list, out_types: list):
        ...
        """
        log_color = Fore.CYAN if method_type == 'query' else Fore.MAGENTA
        print(f"{log_color}EXEC: Wywołanie {method_type.upper()} -> {method_name} z argumentami: {params}")
        ...
        try:
            ...
            response = result_decoded[0] if result_decoded else None
            print(f"{Fore.GREEN}OK: Wywołanie {method_name} zakończone sukcesem. Wynik: {response}")
            return response

        except Exception as e:
            print(f"{Fore.RED}ERROR: Błąd wywołania {method_name}: {e}")
    ...
    async def check_messages(self):
        ...
        if check_result == "PUSH":
            ...
            print(f"{Fore.YELLOW}INFO: Wykryto nową wiadomość. Pobieranie...")
